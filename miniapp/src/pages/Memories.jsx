...
function SummaryTab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await apiFetch(`/api/sessions/${sess.id}/summaries`);
  ...
  if (summaries.length === 0) {
    return <p className="py-16 text-center text-[14px]" style={{ color: S.textMuted }}>暂无记忆摘要</p>;
  }
    ...
    <>
      {summaries.map((s) => (
        ...
        >
          ...
          <p className="text-[12px] leading-relaxed break-words" style={{ color: S.text, maxHeight: 160, overflow: "hidden" }}>
            {s.summary_content || "(空)"}
          </p>
          {s.created_at && (
            <div className="mt-1 text-[10px]" style={{ color: S.textMuted }}>
...
function MessagesTab() {
  ...
  if (messages.length === 0) {
    return <p className="py-16 text-center text-[14px]" style={{ color: S.textMuted }}>暂无消息</p>;
  }
  ...
  return (
    <div className="flex h-full flex-col" style={{ background: S.bg }}>
      ...
      >
        ...
        <h1 className="text-[17px] font-bold" style={{ color: S.text }}>记忆管理</h1>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-8">
        {tab === "summary" ? <SummaryTab /> : <MessagesTab />}
      </div>
    </div>
  );
}
