  ...
  return (
    <div className="relative mb-3 overflow-hidden rounded-[18px]" style={{ background: S.bg, boxShadow: "var(--card-shadow-sm)" }}>
      ...
      <div ref={rowRef} className="relative z-10" style={{ transform: "translateX(0)" }}
        onTouchStart={(e) => { const t = e.touches[0]; const st = s.current; st.sx = t.clientX; st.sy = t.clientY; st.base = st.cur; st.drag = true; st.locked = false; st.horiz = false; if (rowRef.current) { rowRef.current.style.transition = "none"; rowRef.current.style.willChange = "transform"; } if (actRef.current) actRef.current.style.transition = "none"; }}
